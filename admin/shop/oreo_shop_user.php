                                    ...
                                    <div class="page-title-box">
                                        <div class="btn-group float-right">
                                            <ol class="breadcrumb hide-phone p-0 m-0">
                                                ...
                                                <li class="breadcrumb-item active">平台用户</li>
                                            </ol>
                                        </div>
                                        <h4 class="page-title">平台用户管理</h4>
                                    </div>
                                </div>
                            </div>
...
if($_POST['my']==''){
	...
	$sql=" 1";
	$numrows=$DB->query("SELECT count(*) from oreo_shop_user WHERE{$sql}")->fetchColumn();
...
$list=$DB->query("SELECT * FROM oreo_shop_user WHERE{$sql} order by id desc limit $offset,$pagesize");				
while($row = $list->fetch())
{
	                                       if($row['activation']==1){$real_type="<a style='color: red;'>未实名</a>";}
										   if($row['activation']==2){$real_type="<a style='color: green;'>已实名</a>";}
										   if($row['activation']==3){$real_type="<a style='color: blue;'>待审核</a>";}
										   if($row['type']==1){$type="<a style='color: green;'>正常</a>";}else{$type="<a style='color: red;'>封禁</a>";}
                                            echo '<tr >
											 <td >'.$row['user'].'</td>
											 <td >'.$row['name'].'</td>
											 <td >'.$row['qq'].'</td>
											 ...
											 <td style="display: none;">'.$row['activation'].'</td>
                                             ...
                                             <div class="modal-header">
                                               <h5 class="modal-title" id="exampleModalLabel">编辑用户信息</h5>
												...
												<div class="form-group">
                                                    <label>联系电话</label>
                                                    <div>
													<input type="text" class="form-control ca3" name="phone"/>
                                                    </div>
                                                </div>
												<div class="form-group">
                                                    <label>邮箱</label>
												...
												<div class="form-group">
                                                    <label>余额</label>
                                                    <div>
													<input type="text" class="form-control ca8" name="money"/>
                                                    </div>
                                                </div>
												<div class="form-group">
                                                    <label>状态</label>
