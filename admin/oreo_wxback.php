                                ...
                                <div class="col-12">
                                    <div class="card m-b-30">
                                        ...
                                        <div class="col-lg-4">
												...
												<a data-toggle="modal" data-target="#tianjia" data-id="tianjia"  class="btn btn-success mb-2 mr-2"><i class="mdi mdi-bolnisi-cross"></i>添加微信免签</a>
												</div>
                                            </div><!-- end col-->
...
<?php
$sql=" 1";
$numrows=$DB->query("SELECT count(*) from oreo_wxback WHERE{$sql}")->fetchColumn();
...
$list=$DB->query("SELECT * FROM oreo_wxback WHERE{$sql} order by id desc limit $offset,$pagesize");				
...
if ($page>1)
{
echo '<li class="page-item"><a class="page-link" href="oreo_wxback.php?page='.$first.$link.'">首页</a></li>';
echo '<li class="page-item"><a class="page-link" href="oreo_wxback.php?page='.$prev.$link.'">&laquo;</a></li>';
} else {
echo '<li class="page-item"><a class="page-link">首页</a></li>';
}
for ($i=1;$i<$page;$i++)
echo '<li class="page-item"><a class="page-link" href="oreo_qcboreo_wxbackack.php?page='.$i.$link.'">'.$i .'</a></li>';
echo '<li class="page-item active"><a class="page-link">'.$page.'</a></li>';
if($pages>=3)$s=3;
else $s=$pages;
for ($i=$page+1;$i<=$s;$i++)
echo '<li class="page-item"><a class="page-link" href="oreo_wxback.php?page='.$i.$link.'">'.$i .'</a></li>';
echo '';
if ($page<$pages)
{
echo '<li class="page-item"><a class="page-link" href="oreo_wxback.php?page='.$next.$link.'">&raquo;</a></li>';
echo '<li class="page-item"><a class="page-link" href="oreo_wxback.php?page='.$last.$link.'">尾页</a></li>';
} else {
...
<script>
 ...
 $('#shanchu').on('show.bs.modal', function (event) {
      ...
      modal.find('.ca4').val(content); 
						...
						$.ajax({
							type: "POST",
							url: "ajax.php?act=edit_Wxxiugai",
					...
					$("#shanchul").click(function () {
						...
						$.ajax({
							type: "POST",
							url: "ajax.php?act=edit_Wxshanchu",
							data: {ids:ids},
							dataType: 'json',
							success: function (data) {
								layer.close(ii);
					...
					$("#tianjias").click(function () {
						...
						var title=$("input[name='titlet']").val();
						...
						$.ajax({
							type: "POST",
							url: "ajax.php?act=edit_Wxtianjia",
