...
case 'delete_JkSafe':
	...
	$tongbu=$DB->query("select * from oreo_pay_safe where domain='$domain' and token='$token' and user='$user'")->fetch();
	$DB->exec("update `oreo_regcode` set `status` ='1' where `id`='{$row['id']}'");
    $DB->exec("DELETE FROM oreo_pay_safe_log WHERE domain='$domain' AND update_key='{$tongbu['update_key']}' AND user='$user' ");
	$sql = "DELETE FROM oreo_pay_safe WHERE domain='$domain' AND user='$user' ";
    if($DB->exec($sql))
...
$domain=daddslashes(strip_tags($_POST['domain']));
$token=daddslashes(strip_tags($_POST['token']));
$tongbu=$DB->query("select * from oreo_pay_safe where domain='$domain' and token='$token'")->fetch();
...
if($safe_auth != $token_auth)exit('{"code":"-1","msg":"远程返回数据时安全验证未通过"}');
	...
	//分析远程数据并写入数据库
	//首先查询是否有数据判断是否要更新数据
	$jk=$DB->query("select * from oreo_pay_safe_log where (name='weixin' or name='alipay' or name='qq') and user='$user' and update_key='$sysnum' ")->fetch();
	if(!$jk){
		//插入新数据操作
	$DB->exec("INSERT INTO `oreo_pay_safe_log` (`user`, `xieyi`, `domain`, `update_key`, `name`, `jk_mode`, `jk_pass`) VALUES ('{$user}', '{$xieyi}', '{$domain}', '{$sysnum}', 'alipay', '{$ali}', '{$ali_pass}')");
	$DB->exec("INSERT INTO `oreo_pay_safe_log` (`user`, `xieyi`, `domain`, `update_key`, `name`, `jk_mode`, `jk_pass`) VALUES ('{$user}', '{$xieyi}', '{$domain}', '{$sysnum}', 'weixin', '{$wx}', '{$wx_pass}')");
	$DB->exec("INSERT INTO `oreo_pay_safe_log` (`user`, `xieyi`, `domain`, `update_key`, `name`, `jk_mode`, `jk_pass`) VALUES ('{$user}', '{$xieyi}', '{$domain}', '{$sysnum}', 'qq', '{$qq}', '{$qq_pass}')");
	$DB->exec("update `oreo_pay_safe` set  `addtime` ='{$date}' where user='{$user}'");
	exit('{"code":"1","msg":"远程获取数据成功"}');
	}else{
		//更新数据操作
	$DB->exec("update `oreo_pay_safe_log` set  `jk_mode` ='{$ali}', `jk_pass` ='{$ali_pass}' where name='alipay' and user='{$user}' and domain='{$domain}' and update_key='{$sysnum}'");
	$DB->exec("update `oreo_pay_safe_log` set  `jk_mode` ='{$wx}', `jk_pass` ='{$wx_pass}' where name='weixin' and user='{$user}' and domain='{$domain}' and update_key='{$sysnum}'");	
	$DB->exec("update `oreo_pay_safe_log` set  `jk_mode` ='{$qq}', `jk_pass` ='{$qq_pass}' where name='qq' and user='{$user}' and domain='{$domain}' and update_key='{$sysnum}'");
	$DB->exec("update `oreo_pay_safe` set  `addtime` ='{$date}' where user='{$user}' and domain='{$domain}' and update_key='{$sysnum}'");
...
case 'edit_QCtianjia':
	...
	if(!$userid){exit('{"code":-1,"msg":"各项不能留空"}');}
...
case 'edit_Wxtianjia':
	...
	$cxyh=$DB->query("SELECT * FROM `oreo_user` WHERE id='$userid' limit 1")->fetch();
	if(!$cxyh){exit('{"code":-1,"msg":"用户账号不存在"}');}
    $cxurl=$DB->query("SELECT * FROM `oreo_wxback` WHERE url='$url' limit 1")->fetch();
    if($cxurl){exit('{"code":-1,"msg":"该站点已存在"}');}
    $token=ghcode(16); 
	$addtime=date("Y-m-d H:i:s");
	$sds=$DB->exec("INSERT INTO `oreo_wxback` (`token`, `userid`, `title`, `url`, `callback`, `addtime`, `state`) VALUES ('$token', '$userid', '$title', '$url', '$callback', '$addtime', '$state')");
	exit('{"code":1,"msg":"添加成功"}');
break;	
...
case 'edit_QCxiugai':
	...
	$callback=daddslashes(strip_tags($_POST['callback']));
	$state=daddslashes(strip_tags($_POST['state']));
	$sqs=$DB->exec("UPDATE `oreo_qcback` set `title` = '$title', `callback` = '$callback',  `state` = '$state'   WHERE `id` = '$id'");
	exit('{"code":1,"msg":"修改成功"}');	
break;
case 'edit_Wxxiugai':
    $id=daddslashes(strip_tags($_POST['id']));
	...
	$callback=daddslashes(strip_tags($_POST['callback']));
	$state=daddslashes(strip_tags($_POST['state']));
	$sqs=$DB->exec("UPDATE `oreo_wxback` set `title` = '$title', `callback` = '$callback',  `state` = '$state'   WHERE `id` = '$id'");
	exit('{"code":1,"msg":"修改成功"}');	
break;
case 'edit_Sqsxiugai':
    $id=daddslashes(strip_tags($_POST['id']));
...
case 'edit_Wxshanchu':
    $id=daddslashes(strip_tags($_POST['ids']));
	$sql="DELETE FROM oreo_wxback WHERE id='$id'";	
    if($DB->exec($sql))
	 exit('{"code":1,"msg":"succ"}');
    else
	 exit('{"code":-1,"msg":"删除数据失败！"}');
...
case 'edit_Xcxtj':
	...
	$sds=$DB->exec("INSERT INTO `oreo_authsys` (`name`, `syskeys`, `type`, `money`,  `sjzf`, `otable`, `user`,`grade_code1`, `grade_code2`, `grade_code3`) VALUES ('$name', '$syskeyst', '$type', '$moneyt', '$sjzf', '$otable','$user', '$grade_code1', '$grade_code2', '$grade_code3')");
        $pid=$DB->lastInsertId();
        if($sds){
		 exit('{"code":1,"msg":"添加成功" }');
    }else{
...
case 'edit_Xcxxg':
    ...
    $sqs=$DB->exec("UPDATE `oreo_authsys` SET `name` = '$name',  `type` = '$type',  `money` = '$money'  WHERE `id` = '$id'");
	exit('{"code":1,"msg":"succ"}');
break;
case 'edit_Xckami':
