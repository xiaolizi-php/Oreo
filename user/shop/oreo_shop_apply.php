...
$row=$DB->query("SELECT * FROM oreo_shop_details WHERE out_trade_no='{$_POST['id']}' AND status='1' AND shop_type='0'  AND user='{$pid}' limit 1")->fetch();
...
$DB->exec("update `oreo_shop_details` set `shop_type`='4'  where `user`='{$pid}' and out_trade_no='{$_POST['id']}' ");
if($row['name']=="oreo实名认证"){
...
$DB->exec("update `oreo_shop_details` set `shop_type`='5'  where `user`='{$pid}' and out_trade_no='{$_POST['id']}' ");
//发送退款异常的手机短信
$user_info=$DB->query("select * from oreo_shop_user where `user`='{$pid}' limit 1")->fetch();
$sms_type='3';//短信类型
$user_name=$user_info['user'];//用户账号
